INSERT INTO
      sections (
            id,
            topic_id,
            title,
            description,
            what_i_learned
      )
VALUES
      (
            1,
            1,
            '{"fr":"Moi","en":"Me"}',
            '{
                  "fr":"Au commencement, c\’est l\’histoire d\’un jeune adolescent qui veut vivre de sa passion: le football. Il s\’en donne les moyens, grandit à tous les niveaux et prend conscience de la réelle faisabilité de ce projet.
                  Comme vous le savez déjà, je m\’appelle NGUESSIE Loïc. J\’ai 23 ans et vis en Île de France à Nanterre (Département des Hauts-de-Seine).
                  Passé par le collège Paul Eluard à Nanterre (ville du département des Hauts-de-Seine), je découvre assez rapidement mon appétence pour les mathématiques. J\’en ai fait par la suite une priorité dans mon parcours scolaire.
                  En parallèle avec les cours, ma passion prenait une place importante dans ma jeune vie d\’adolescent. Elle régulait et rythmait mes journées, mon emploi du temps ainsi que mes centres d\’intérêts. Malheureusement, tout le monde n\’a pas la chance d\’en faire son “gagne-pain”, qui-plus-est lorsqu\’il s\’agit du football, en région parisienne. Ayant rapidement pris conscience de cette situation, j\’ai décidé de concentrer l\’entièreté de mon temps à mes études. Ajouté à cela, mon organisation, ma volonté d\’atteindre mes objectifs ainsi que mon audace, j\’ai aujourd\’hui l\’ambition d\’être à la tête d\’une direction de service de système d\’information au sein ma propre structure.
                  ",
                  "en":"-"
            }',
            '{
                  "fr":"À propos de mes hobbies, comme vous pouvez le voir j’aime la musique surtout le rap, l’un des mes albums favoris est : chronic 2001, de Dr DRE, l’un des plus grands producteurs de l’industrie musicale. Mes temps libres sont aussi très souvent comblés, par des projets personnels dans des langages logiciels ou web sur lesquels je me forme.
                  De plus, l’une de mes activités favorites est la lecture. En effet, étant vice-président de l’association étudiante ESMA (Etudiants de la Sorbonne pour les Mondes Africains), j’ai l’opportunité de co-diriger un club de lecture depuis 2 ans maintenant. Comme cela va de pair, j’aime aussi les voyages. J’ai eu l’opportunité de voyager très tôt en direction de l’Afrique, vers mon pays d’Origine, mais aussi dans de nombreuses villes françaises, grâce à ma première passion: le football. Mais aussi dans l’Europe.

                  Comme précisé en page d’accueil, je vous détaillerai mon parcours. Ça risque peut-être d’être long mais j’espère que ça vous plaira, alors avant de commencer madame, monsieur, je vous souhaite une belle expérience ! ✌🏿
                  ",
                  "en":"-"
            }'
      ),
      (
            2,
            2,
            '{"fr":"Parcours scolaire","en":"School"}',
            '{"fr":"-","en":"-"}',
            '{"fr":"-","en":"-"}'
      ),
      (
            3,
            3,
            '{"fr":"Le monde professionnel","en":"Work"}',
            '{
                  "fr":"Durant mon parcours universitaire, je voulais savoir à quoi mon domaine d\’étude ressemblait dans la sphère professionnelle. Prendre conscience de la rigueur et de l’organisation lors de la construction d’un projet était mon objectif. Malheureusement, je n\’ai pas tout de suite eu la chance de trouver une expérience dans mon domaine. Pas de panique, tout vient à point à qui sait attendre.

                  Fin de la première année universitaire en mai 2018, je n\'ai toujours pas de réponse positive de la part des entreprises que je ciblais, donc toujours pas d’entretien. À ce moment, il ne sert à rien de s\’affoler puisque l’expérience professionnelle est encore non-exhaustive pour le passage en année supérieure.
                  Je ne me voyais pas faire plus de 3 mois à la maison sans rien faire, alors je ne cherchais plus de \’stage de développeur, data analyst\’ ou d’autres intitulés relatifs à ma spécialités. Je cherchais un job d\'été.
                  ",
                  "en":"-"
            }',
            '{"fr":"-","en":"-"}'
      ),
      (
            4,
            4,
            '{"fr":"Projes personnels","en":"Personal projects"}',
            '{
                  "fr":"Pendant mes débuts universitaires, il me fallait un temps d’adaptation pour prendre conscience de la rigueur attendue pour réussir mes années mais surtout de la discipline à appliquer chaque jour. Cela impliquait alors que mes habitudes sans parler de ma vie universitaire devaient changer. Tout ceci, pour expliquer que ces changements m\’ont permis de libérer du temps pour des projets extra-scolaires.
                  ",
                  "en":"-"
            }',
            '{"fr":"-","en","-"}'
      );